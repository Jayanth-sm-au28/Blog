    ...
    <div className="flex lg:flex-row mt-20 flex-col lg:gap-20 gap-6 md:p-10 justify-between items-start bg-white p-4 rounded-md">
      <div className="flex flex-col gap-4 basis-1/2">
        <div className="flex flex-col gap-3">
          <h2 className="lg:text-7xl text-5xl text-primary font-semibold text-[#6938Ef]">{data.leftHeader}</h2>
          <h3 className="text-2xl font-semibold border-b pb-4 md:text-2xl border-[#4A1FB8] text-left">{data.leftHeaderDescription}</h3>
        </div>
        {data.divider && <div className="h-[1px] w-full bg-primary"></div>}
        <div className="flex flex-col gap-4">
          <p className="text-gray-800 font-medium md:text-xl">{data.leftDescripition}</p>
          <Link
      ...
      <div className="md:bg-primary bg-[#4A1FB8] rounded-xl p-8 flex flex-col justify-between items-start text-start gap-24 basis-2/3">
        <p className=" text-white md:text-xl font-normal">{data.rightHeaderDescripition}</p>
